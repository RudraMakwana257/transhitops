  ...
  const fetchAll = async () => {
    ...
    try {
      const [vRes, tRes, mRes, fRes] = await Promise.all([
        api.get(`/vehicles/${id}`),
        api.get(`/vehicles/${id}/trips?page_size=10`),
        api.get(`/vehicles/${id}/maintenance?page_size=10`),
        api.get(`/vehicles/${id}/fuel?page_size=10`),
      ])
      ...
      if (fRes.data.success) setFuel(fRes.data.data)
    } catch (err) {
          ...
          <CardContent>
            <DataTable
              columns={[
                ...
                { key: 'scheduled_date', header: 'Scheduled', accessor: 'scheduled_date', sortable: true, render: (m: MaintenanceLog) => m.scheduled_date ? format(new Date(m.scheduled_date), 'MMM d, yyyy') : '—' },
