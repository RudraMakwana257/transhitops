      ...
      <PageWrapper title="Dashboard" description="Real-time fleet operations overview">
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="card p-6 animate-pulse"><div className="h-64 bg-[var(--bg-hover)] rounded" /></div>
            <div className="card p-6 animate-pulse"><div className="h-64 bg-[var(--bg-hover)] rounded" /></div>
          </div>
        </div>
      </PageWrapper>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Link to="/trips" className="text-sm text-[var(--brand-primary)] hover:underline flex items-center gap-1 whitespace-nowrap">
  ...
  const renderSafetyOfficer = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader><CardTitle>Driver Safety Overview</CardTitle></CardHeader>
          <CardContent>
            {safetyKpis ? (
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 rounded-lg bg-[var(--bg-sidebar)]">
                  <span className="text-sm text-[var(--text-secondary)]">Avg Safety Score</span>
                  <span className="text-xl font-bold" style={{ color: 'var(--status-available)' }}>{safetyKpis.avg_safety_score}</span>
                </div>
              ...
              <div className="space-y-4 animate-pulse">
                <div className="h-12 bg-[var(--bg-sidebar)] rounded-lg" />
                <div className="space-y-2">{['excellent','good','fair'].map(l => <div key={l} className="flex items-center gap-3"><div className="w-3 h-3 rounded-full bg-[var(--bg-sidebar)]" /><div className="h-4 bg-[var(--bg-sidebar)] rounded flex-1" /></div>)}</div>
              </div>
            )}
          </CardContent>
        </Card>
    ...
    <div className="space-y-6">
            ...
            {financialKpis?.cost_by_type?.length ? (
              <div className="space-y-3">
                {financialKpis.cost_by_type.map((item: any) => (
                  <div key={item.type} className="flex items-center justify-between p-3 rounded-lg bg-[var(--bg-sidebar)]">
      ...
      <Card>
        ...
        <CardContent className="p-0 sm:p-6">
              ...
              <table className="w-full data-table text-sm">
                ...
                <tbody>
                  {financialKpis.top_vehicles.map((v: any) => (
                    <tr key={v.id} className="border-t border-[var(--border-default)]/50 hover:bg-[var(--bg-hover)]/50 transition-colors">
                      <td className="data-table td font-medium">{v.name}</td>
                      <td className="data-table td font-mono text-[var(--text-muted)]">{v.reg_number}</td>
          ...
          ) : <p className="text-sm text-[var(--text-muted)]">No vehicle cost data</p>}
        </CardContent>
      </Card>
    </div>
  )
